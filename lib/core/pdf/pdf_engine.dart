...
class PdfEngineNotifier extends Notifier<PdfEngineState> {
  ...
  Future<void> init(String newCantoId) async {
    ...
    try {
      ...
      final canto = cantos.where((c) => c.id == newCantoId).firstOrNull;
      if (canto == null || canto.archivo.isEmpty) {
        state = state.copyWith(
          isLoading: false,
          error: 'Partitura no encontrada en el catálogo',
        );
        return;
      }

      final downloaded = await OfflineFiles.ensurePdf(canto);
      ...
      // Refrescar el indicador de caché offline del drawer
      ref.read(syncManagerProvider.notifier).refresh();
    } catch (e) {
      state = PdfEngineState(
        cantoId: newCantoId,
        isLoading: false,
        error: 'Partitura no disponible. Verifica tu conexión a internet.',
      );
    } finally {
      _isInitializing = false;
      _initializingForId = null;
    }
  }
